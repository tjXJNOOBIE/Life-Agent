...
const source = resolve(root, 'build', 'install', 'life-agent')
...
if (result.status !== 0) throw new Error(`Life Agent Java distribution failed with exit code ${result.status}`)
...
writeFileSync(resolve(root, 'runtime-manifest.json'), `${JSON.stringify({
  ...
  distribution: 'life-agent',
  launchers: ['life-agent'],
  treeSha256: tree.digest('hex'),
  files: entries,
}, null, 2)}\n`)
console.log(`Packaged Life Agent Java runtime: ${entries.length} files, ${entries.reduce((sum, entry) => sum + entry.bytes, 0)} bytes`)
