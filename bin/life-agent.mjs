...
const nodeEnvironment = 'LIFE_AGENT_STRANDS_NODE'
const entrypointEnvironment = 'LIFE_AGENT_STRANDS_ENTRYPOINT'
...
const assertPackage = () => {
  if (!existsSync(runtime) || !existsSync(manifestPath)) throw new Error('Java runtime bundle is missing; install a published Life Agent package or build the release package with npm pack.')
  const manifest = JSON.parse(readFileSync(manifestPath, 'utf8'))
  if (manifest.product !== 'life-agent') throw new Error('Life Agent runtime manifest has the wrong product identity.')
  ...
  if (JSON.stringify(actual) !== JSON.stringify(manifest.files)) throw new Error('Life Agent runtime integrity verification failed.')
  const tree = createHash('sha256')
  for (const entry of actual) tree.update(`${entry.path}\0${entry.sha256}\n`)
  if (tree.digest('hex') !== manifest.treeSha256) throw new Error('Life Agent runtime tree verification failed.')
  return manifest
}
...
const assertJava = () => {
  ...
  if (result.error || result.status !== 0) throw new Error('Java 25 or newer is required to run the Java Life Agent backend.')
...
const doctor = () => {
  ...
  console.log('Life Agent doctor: PASS')
  ...
  console.log('MCP: Java stdio/HTTP planning surfaces and seven MCP App resources are packaged')
  console.log('Providers/authentication: not inspected by the package doctor')
}
...
const launch = (args) => {
  ...
  assertJava()
  const bridge = resolveBridgeEntrypoint()
  const backend = resolve(runtime, 'bin', `life-agent${process.platform === 'win32' ? '.bat' : ''}`)
  ...
  const child = spawn(backend, args, { cwd: process.cwd(), env: environment, stdio: 'inherit', shell: process.platform === 'win32', windowsHide: false })
