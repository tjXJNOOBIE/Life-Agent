...
5. Keep the bundled MCP server limited to orchestration UI, user-owned durable state, and bounded UI-asset resolution/cache behavior explicitly used by those surfaces. The asset layer may cache sanitized brand icons and curated contextual backgrounds, but it must not become a general proxy or provider-data store.
